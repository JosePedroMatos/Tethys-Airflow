...
# The NUREK_RESERVOIR driver reads its input workbook directly from a host data folder
# (NUREK_DATA_FOLDER). build_mounts only binds the LOCAL/STORAGE output tiers, so bind-mount the
# input folder read-only and rewrite the env var the driver reads so that inside the container
# os.getenv('NUREK_DATA_FOLDER') resolves to the mounted target, not the raw host path.
container_mounts = container_mounts + [
    Mount(
        source=series_config['NUREK_DATA_FOLDER'],
        target=series_config['NUREK_DATA_FOLDER_DOCKER'],
        type='bind',
        read_only=True,
    ),
]
container_env['NUREK_DATA_FOLDER'] = series_config['NUREK_DATA_FOLDER_DOCKER']
...
with DAG(
    'tethys_nurek_pipeline',
    default_args=default_args,
    description='Pipeline to retrieve Nurek reservoir data via tethys-series container',
    ...
    t1 = DockerOperator(
        task_id='retrieve_nurek',
        command=make_command('NUREK_RESERVOIR'),
        **common_docker_args,
    )
