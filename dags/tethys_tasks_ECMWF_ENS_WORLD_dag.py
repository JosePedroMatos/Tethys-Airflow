...
'''
docker-compose run --rm tethys-tasks ECMWF_ENS_TP_WORLD retrieve --class_kwargs "{\"date_from\": \"'2025-05-01'\", \"download_from_origin\": \"True\"}"
'''
...
with DAG(
    f'tethys_ecmwf_ens_{zone.lower()}_pipeline',
    default_args=default_args,
    description='Pipeline to retrieve ECMWF ENS world data via tethys-tasks container',
    schedule_interval=schedule_interval,
    catchup=False,
    max_active_runs=1,
    tags=['tethys', 'ecmwf', 'ens', 'tasks'] + zone_tags,
) as dag:

    date_from = (pd.Timestamp.now() - pd.Timedelta('2d')).strftime('%Y-%m-%d')
    print(f'Attempting retrieve from {date_from}.')
    ...
    t1 = DockerOperator(
        ...
        command=make_command(f'ECMWF_ENS_T2M_{zone}'),
        **common_docker_args,
    )

    t2 = DockerOperator(
        task_id='retrieve_tp_data',
        command=make_command(f'ECMWF_ENS_TP_{zone}'),
        **common_docker_args,
    )

    t3 = DockerOperator(
        task_id='retrieve_sd_data',
        command=make_command(f'ECMWF_ENS_SD_{zone}'),
        **common_docker_args,
    )
