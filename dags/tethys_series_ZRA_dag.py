...
with DAG(
    ...
    description='Pipeline to retrieve Zambezi River Authority hydrology data via tethys-series container',
    schedule_interval=schedule_interval,
    catchup=False,
    max_active_runs=1,  # Only run one instance at a time, skips backlog
    tags=['tethys', 'series', 'zra'],
) as dag:
    ...
    # ZRA_RIVER_FLOWS and ZRA_RESERVOIR scrape independent pages of the same site into their own
    # SOURCE/LOCAL/STORAGE trees, so unlike METEOSUISSE there is nothing to share -- both fetch
    # from origin and can run in parallel.
    t1 = DockerOperator(
        task_id='retrieve_zra_river_flows',
        command=make_command('ZRA_RIVER_FLOWS'),
        **common_docker_args,
    )

    t2 = DockerOperator(
        task_id='retrieve_zra_reservoir',
        command=make_command('ZRA_RESERVOIR'),
        **common_docker_args,
    )
